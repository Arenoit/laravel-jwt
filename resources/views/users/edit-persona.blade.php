...
<?php
    $id = request()->id;
    $persona = \App\Models\Personas::find($id); // o usar where si necesitas más control
?>

    <form class="js-validate" action="{{ route('edit-1persona') }}" method="post" enctype="multipart/form-data"
        id="form-id">
        @csrf
        @method('PUT')
                        ...
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <input type="text" id="nombre" name="nombre" class="form-control __form-control"
                                    placeholder="Nombre de la Persona" value="{{ $persona->nombre }}" required>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <input type="email" id="email" name="email" class="form-control __form-control"
                                    placeholder="Correo electrónico" value="{{ $persona->email }}" required>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <input type="date" id="fecha_n" name="fecha_n" class="form-control __form-control"
                                    placeholder="Fecha de nacimiento" value="{{ $persona->fecha_de_nacimiento }}" required>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="text-end pt-3">
            <a href="{{ route('personas') }}" class="cmn--btn rounded-md border-0 text-white px-3 py-2" style="background-color: #6c757d;">Regresar</a>
